...
import { CalendarClock, Video, MapPin, Copy, Users } from "lucide-react";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
export function TutorUpcomingSessions({ bookings: localBookings, groupBookings }) {
  const { user } = useAuth();
  ...
  //     const response = await axios.get(
  //       `http://localhost:5000/api/group-sessions/tutor/${user.id}`
  //     );
          ...
          >
            <div className="flex-1 space-y-1">
              <div className="flex items-center gap-2">
                {booking.sessionType === "group" ? (
                  <div className="h-10 w-10 rounded-full bg-[#3b2762] flex items-center justify-center cursor-pointer">
            ...
            <div className="flex items-center gap-2">
              {sessionStatus.status === "ended" && (
                <CustomTooltip
                  content="Declare this session complete in Sessions tab"
                  position="left"
                >
                  <Badge
                    variant="outline"
